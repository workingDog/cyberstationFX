  ...
  def createSRORel(x: SRO) = {
    ...
    def baseRel(sourceId: String, targetId: String, name: String): Option[org.neo4j.graphdb.Relationship] = {
      ...
      val relationOpt = transaction {
        ...
        val targetNode = Neo4jDbService.idIndex.get("id", targetId).getSingle
  ...
  def createStixObjRel(stixObj: StixObj) = {
    stixObj match {
      ...
      case x: LanguageContent =>
        ...
        transaction {
          val sourceNode = Neo4jDbService.idIndex.get("id", x.id.toString()).getSingle
          val targetNode = Neo4jDbService.idIndex.get("id", x.object_ref.toString()).getSingle
